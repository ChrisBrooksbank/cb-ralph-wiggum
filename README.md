...
Ralph Wiggum is a bash loop technique that feeds prompts to Claude repeatedly, with each iteration getting **fresh context**. Progress is stored in files and git history, not in the LLM context window.
...
3. Generate all Ralph Wiggum files:
   - `loop.sh` - Bash loop orchestrator
...
```
your-project/
├── loop.sh                    # Bash loop (runs Claude repeatedly)
...
### Phase 3: Building (`./loop.sh build`)
